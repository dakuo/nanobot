...
def _exchange_code_for_token_async(
    ...
    async def _run() -> OAuthToken:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            if provider.token_content_type == "json":
...
def _refresh_token(refresh_token: str, provider: OAuthProviderConfig) -> OAuthToken:
    ...
    access, refresh, expires_in = _parse_token_payload(
        payload, "Token refresh response missing fields"
    )
...
async def _await_manual_input(print_fn: Callable[[str], None]) -> str:
    print_fn(
        "[cyan]Paste the authorization code (or full redirect URL), or wait for the browser callback:[/cyan]"
    )
...
def login_oauth_interactive(
    ...
    async def _login_async() -> OAuthToken:
        ...
        # Add provider-specific extra params (backward-compat: OpenAI Codex
        # provider defines its own extras; legacy hardcoded params removed).
        ...
        print_fn(
            "[cyan]A browser window will open for login. If it doesn't, open this URL manually:[/cyan]"
        )
        print_fn(url)
        try:
            webbrowser.open(url)
