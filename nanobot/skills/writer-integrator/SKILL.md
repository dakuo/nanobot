...
- **Introduction**: problem → gap → contribution statement → paper structure overview.
  - **5-element arc** (from PI's 6 highly-cited HCI papers): (1) Problem grounding with concrete evidence/statistics; (2) Explicit gap identification with challenges woven into prose (use inline numbering like "First,...Second,...Third,..." or "(1)...; (2)...; (3)..." WITHIN a prose paragraph, NEVER as standalone numbered list items with bold headers on separate lines). When introducing type taxonomies or classifications (e.g., two types of LLM agents), use *italic* for category names: "*LLM-based autonomous agents*" and "*digital twins of human participants*"; (3) "In this work, we..." bridge introducing the system; (4) System components as inline enumeration within a prose paragraph — each component MUST have a **bolded name** (e.g., **Persona Generator**, **Universal Browser Connector**) and a substantive "what it does" description (1-2 clauses explaining the component's function for the researcher, NOT implementation details). (**MUST cross-check count and names against outline.md / user_input.md**); (5) Contributions as standalone numbered list (this is the ONE AND ONLY exception where a list is acceptable in the Introduction).
  - **ABSOLUTE LIST BAN**: The final contribution list is the ONLY standalone list permitted in the Introduction. Requirements, gaps, challenges, and all other enumerations MUST be inline within prose paragraphs. The format `1. **Bold header.** Description` on separate lines is a standalone list and is BANNED.
  ...
  - **Contribution list format**: ALWAYS format contributions as a standalone bulleted or numbered list, not inline prose. Introduce with "We make the following contributions:" (preferred) or "This paper presents the following contributions:". Each item gets 1–2 sentences on its own line. Typical count: 3–4 items.
...
- **No forbidden sentence structures**: **No em-dashes (—) anywhere in generated prose.** This is an absolute ban covering ALL forms: paired parentheticals (`— phrase —`), single asides (`X — Y`), appositives (`X — an approach that`), and list introductions (`X — namely, Y`). Replace with commas, periods, colons, "which"/"that" clauses, or parentheses. Scan every merged draft for the literal character — (U+2014) before delivering. Also banned: **"is not to X but to Y" / "is not X but Y" / "X rather than Y"** (all defensive negation framing variants; state the positive purpose directly instead), trailing participial phrases (`, verb-ing xxx`), comma+gerund clauses (`, having xxx`), **absolute/imperative words** ("must", "absolutely", "certainly", "undeniably" in paper prose; replace with "needs to", "requires", "should"), **categorical claims** ("No existing system...", "All prior work..."; soften to "Most existing...", "Few prior..."), **standalone bullet/numbered lists in Introduction body** (except final contribution list; convert to inline prose). See `writing_voice.md` "Forbidden Sentence Structures" for the full rule with examples.
...
2. **Search for "is not to" / "is not X but" / "rather than" (defensive negation)**: Search for "is not to", "is not X but", and "rather than" used in defensive framing (contrasting what something is vs. isn't). If found → rewrite to state the positive purpose directly. Remove the negation entirely.
...
5. **Search for absolute/imperative words**: Scan for "must" (not in formal spec citations), "absolutely", "undeniably", "certainly", "unquestionably", "invariably". If found → replace with softer alternatives ("needs to", "requires", "should", or restructure).
6. **Search for categorical claims**: Scan for "No existing", "No current", "All prior", "No prior", "never" (universal), "always" (universal). If found → soften with scope qualifiers ("Most existing", "Few prior", "typically", "in most cases").
7. **Inter-paragraph cohesion check**: For each paragraph boundary, verify the first sentence of the new paragraph explicitly connects to the closing claim of the previous paragraph. If the connection is implicit or absent, rewrite the opening sentence to echo a key term or concept from the prior paragraph's conclusion.
8. **Bullet/list check**: Scan for standalone numbered or bulleted lists ANYWHERE in the Introduction body (excluding the final contribution list). This includes: lines starting with `1.`, `2.`, `3.`, `-`, `*`, or `•` followed by bold headers or descriptions. Also includes `1. **Bold header.** Description` format. If found → convert to inline prose with "First, ... Second, ... Third, ..." or "(1)... (2)... (3)..." woven into a single flowing paragraph. The ONLY standalone list allowed is the final contribution list.
