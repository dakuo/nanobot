...
- **Introduction**: problem → gap → contribution statement → paper structure overview.
  - **5-element arc** (from PI's 6 highly-cited HCI papers): (1) Problem grounding with concrete evidence/statistics; (2) Explicit gap identification with challenges woven into prose (use inline numbering like "First,...Second,...Third,..." not standalone lists); (3) "In this work, we..." bridge introducing the system; (4) System components as inline enumeration within a prose paragraph (**MUST cross-check count and names against outline.md / user_input.md**); (5) Contributions as standalone numbered list (this is the ONE exception where a list is acceptable in the Introduction).
...
8. **Bullet/list check**: Scan for standalone numbered or bulleted lists in the Introduction body (excluding the final contribution list). If found → convert to inline prose with "First, ... Second, ... Third, ..." or "(1)... (2)... (3)..." woven into sentences.
