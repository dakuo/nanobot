...
def onboard():
    ...
    if typer.confirm("Enable mem0 semantic memory?", default=False):
        ...
        save_config(config)
        console.print(
            "[green]✓[/green] mem0 enabled. Make sure Ollama is running with the embedding model."
        )
        console.print(
            "  [dim]Pull models: ollama pull nomic-embed-text && ollama pull llama3.2[/dim]"
        )
...
def gateway(
    ...
    agent = AgentLoop(
        ...
        exa_api_key=config.tools.web.search.exa_api_key or None,
    ...
    if web_cfg.enabled:
        from nanobot.http.server import HTTPServer

        http_server = HTTPServer(
            agent=agent,
            host=web_cfg.host or "127.0.0.1",
            port=port,
            token=web_cfg.token,
        )
        console.print(f"[green]✓[/green] HTTP API: http://{http_server.host}:{port}")
...
def _login_anthropic_oauth() -> None:
    try:
        ...
        if not (token and token.access):
            ...
            console.print(
                "[dim]A browser will open. Log in with your Claude Pro/Max account.[/dim]"
            )
            console.print(
                "[dim]After authorization, copy the code shown on the page and paste it here.[/dim]\n"
            )
            token = login_oauth_interactive(
                print_fn=lambda s: console.print(s),
                prompt_fn=lambda s: typer.prompt(s),
        ...
        if not (token and token.access):
            console.print("[red]✗ Authentication failed[/red]")
            raise typer.Exit(1)
        console.print("[green]✓ Authenticated with Anthropic (Claude Pro/Max)[/green]")
    except ImportError:
        console.print("[red]oauth_cli_kit not installed. Run: pip install oauth-cli-kit[/red]")
        raise typer.Exit(1)
